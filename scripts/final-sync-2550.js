...
// NFO Lot Sizes for indices
const NFO_LOT_SIZES = {
    'NIFTY': 50, 'BANKNIFTY': 50, 'FINNIFTY': 50, 'MIDCPNIFTY': 50, 'SENSEX': 10
};

const finalSync2550 = async () => {
    let connection = null;
    try {
        console.log('\n🎯 FINAL SYNC - EXACTLY 2550 SCRIPTS USED IN APP\n');
        ...
        // Load NSE watchlist from file (453 scripts actually used in app)
        console.log('[1/6] 📥 Loading NSE watchlist (scripts used in app)...');
        ...
        console.log('   📍 NSE: Collecting watchlist stocks...');
        ...
        // 2. NFO - All stocks from watchlist + index options
        ...
        for (const baseSymbol of nseWatchlist) {
            const nfoMatches = Object.keys(instrumentsBySymbol)
                ...
                .slice(0, 20); // 20 variations per stock
        ...
        for (const indexName of NFO_INDEX_UNDERLYINGS) {
            const indexMatches = Object.keys(instrumentsBySymbol)
                ...
                .slice(0, 100); // Get more for index options
        ...
        for (const baseSymbol of MCX_USED) {
            const mcxMatches = Object.keys(instrumentsBySymbol)
                ...
                .slice(0, 15); // 15 expiries per base
        ...
        console.log(`   📊 Total collected: ${toInsert.length}`);
        ...
        } else if (toInsert.length < targetCount) {
            console.log(`\n   ⚠️  Only ${toInsert.length} available (less than target 2550)`);
        }
        ...
        console.log('✅ DONE! Synced exactly the scripts used in app!\n');
